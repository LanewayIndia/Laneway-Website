            ...
            {milestones.map((milestone, index) => (
              ...
              >
                <div className={`flex-1 ${index % 2 === 0 ? "md:text-right" : "md:text-left"}`}>
                  <div className="relative glass-card rounded-xl p-6 inline-block overflow-hidden transition-all duration-500 hover:border-gold/20">

                    {/* Background animations */}
                    {milestone.year === "The Beginning" && (
                      <motion.div
                        className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(201,168,85,0.12),transparent_60%)]"
                        animate={{ opacity: [0.3, 0.6, 0.3] }}
                        transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
                      />
                    )}

                    {milestone.year === "The Spark" && (
                      <motion.div
                        className="absolute inset-0 bg-[linear-gradient(120deg,transparent,rgba(201,168,85,0.15),transparent)]"
                    ...
                    {milestone.year === "The Mission" && (
                      <motion.div
                        className="absolute inset-0 bg-[linear-gradient(rgba(201,168,85,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(201,168,85,0.05)_1px,transparent_1px)]"
                    ...
                    {milestone.year === "Lessons Learned" && (
                      <motion.div
                        className="absolute inset-0 bg-[radial-gradient(circle_at_50%_100%,rgba(201,168,85,0.1),transparent_70%)]"
                        animate={{ y: [0, -20, 0] }}
                        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
                      />
                    )}

                    {milestone.year === "Our Commitment" && (
                      <motion.div
                        className="absolute inset-0 border border-gold/10 rounded-xl"
                        animate={{ scale: [1, 1.03, 1] }}
                        transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
                      />
                    )}

                    {milestone.year === "The Future" && (
                      <motion.div
                        className="absolute inset-0 bg-[linear-gradient(90deg,transparent,rgba(201,168,85,0.12),transparent)]"
                        animate={{ x: ["-100%", "100%"] }}
