...
export default function JobAdmin() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setJobs(savedJobs);
      nextId.current = savedJobs.length + 100;
    }
  }, []);

  // Persist jobs to localStorage after changes
  useEffect(() => {
    localStorage.setItem('laneway-jobs', JSON.stringify(jobs));
  }, [jobs]);
    ...
    setJobs(currentJobs => {
      if (editJob) {
        const updated = currentJobs.map(j => 
          j.id === editJob.id 
            ? { ...j, title: form.title, location: form.location || j.location, type: form.type, salary: form.salary || j.salary, description: form.description, requirements, tags, postedDate: now }
            : j
        );
        localStorage.setItem('laneway-jobs', JSON.stringify(updated));
  ...
  const deleteJob = (id: number) => {
    setJobs(current => {
      const newJobs = current.filter(j => j.id !== id);
      localStorage.setItem('laneway-jobs', JSON.stringify(newJobs));
      return newJobs;
    });
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .modal-content {
          ...
          border-radius: 16px; padding: 2rem; width: 560px; max-height: 90vh; overflow-y: auto;
